...
for abi in ${APP_ABI[*]}; do
        ...
        armeabi-v7a)
                ...
                compile $abi "$NDK_ROOT/platforms/android-24/arch-arm" "$NDK_ROOT/toolchains/arm-linux-androideabi-4.9/prebuilt/$host-x86_64/bin" "arm-linux-androideabi" "-march=armv7-a -mfloat-abi=softfp -mfpu=neon"
                ;;
        x86)
                # http://gcc.gnu.org/onlinedocs/gcc/x86-Options.html
                compile $abi "$NDK_ROOT/platforms/android-24/arch-x86" "$NDK_ROOT/toolchains/x86-4.9/prebuilt/$host-x86_64/bin" "i686-linux-android" "-march=i686"
                ;;
        arm64-v8a)
                # https://gcc.gnu.org/onlinedocs/gcc/AArch64-Options.html#AArch64-Options
                compile $abi "$NDK_ROOT/platforms/android-24/arch-arm64" "$NDK_ROOT/toolchains/aarch64-linux-android-4.9/prebuilt/$host-x86_64/bin" "aarch64-linux-android" "-march=armv8-a"
                ;;
        *)
                echo "Error APP_ABI"
                exit 1
