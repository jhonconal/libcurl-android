...
compile() {
        ...
        ./Configure android shared --openssldir=$BUILD_PATH/openssl/$ABI
        checkExitCode $?
        # clean
        make clean
        checkExitCode $?
...
for abi in ${APP_ABI[*]}; do
        case $abi in
        armeabi-v7a)
                compile $abi "$NDK_ROOT/platforms/android-24/arch-arm" "$NDK_ROOT/toolchains/arm-linux-androideabi-4.9/prebuilt/$host-x86_64/bin" "armv7" "android" "arm" "arm-linux-androideabi-"
                ;;
        x86)
                compile $abi "$NDK_ROOT/platforms/android-24/arch-x86" "$NDK_ROOT/toolchains/x86-4.9/prebuilt/$host-x86_64/bin" "i686" "android" "x86" "i686-linux-android-"
                ;;
        arm64-v8a)
                compile $abi "$NDK_ROOT/platforms/android-24/arch-arm64" "$NDK_ROOT/toolchains/aarch64-linux-android-4.9/prebuilt/$host-x86_64/bin" "armv8" "android64" "arm" "aarch64-linux-android-"
                ;;
        *)
                echo "Error APP_ABI"
                exit 1
